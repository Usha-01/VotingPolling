...
import MiniDrawer from './AdminPage';
import Box from '@mui/material/Box';
import logo from "../Image/Candidateadd.mp4";
...
      }); 
  ...
  const handleUpdateCandidate = async (index) => {
    try {
      const response = await axios.put('http://localhost:8080/candidate/put/{id}', newCandidate,{ // Replace '/api/candidates' with your backend API endpoint URL
 ...
      },
      ...
      setCandidates((updatedCandidates)=>[...updatedCandidates, response.data]);
      setNewCandidate({
        firstName: '',
        lastName: '',
        partyName: '',
